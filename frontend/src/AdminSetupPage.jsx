...
//   title:    bölmənin başlığı
//   columns:  istifadəçiyə göstəriləcək sütun təsviri
//   accept:   input accept atributu
// ─────────────────────────────────────────────────────────────
function ImportCard({ title, endpoint, columns, accept = ".xlsx,.xls,.csv" }) {
  const inputRef = useRef(null);
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState(null); // { ok, message, inserted, failed, errors }
    ...
    <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
      ...
      {result && (
        ...
        >
          ...
          {result.ok && (result.inserted > 0 || result.failed > 0) && (
            <div className="mt-1 text-white/50">
              Əlavə edildi: <span className="text-white/80">{result.inserted}</span>
              {result.failed > 0 && (
                <>
                  {" · "}Xəta: <span className="text-red-300">{result.failed}</span>
                </>
              )}
            </div>
          )}
          {result.errors && result.errors.length > 0 && (
            <details className="mt-2">
              <summary className="cursor-pointer text-white/60 hover:text-white/80">
                Xətaları göstər ({result.errors.length})
              </summary>
              <ul className="mt-1.5 space-y-0.5 max-h-40 overflow-y-auto">
                ...
                {result.errors.length > 50 && (
                  <li className="text-white/40">... və {result.errors.length - 50} daha</li>
                )}
              </ul>
            </details>
          )}
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="mt-6">
          <button
            ...
            className="w-full flex items-center justify-between px-5 py-3.5 rounded-2xl bg-white/5 border border-white/10 text-white/70 hover:text-white hover:bg-white/10 transition-colors"
          >
